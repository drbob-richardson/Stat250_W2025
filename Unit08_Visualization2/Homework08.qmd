...
The built-in `Orange` dataset records the growth (circumference) of orange trees measured at different ages. Create a line plot with the following specifications: - Plot **age** on the x-axis and **circumference** on the y-axis. - Draw one line per tree (group the data by `Tree`), using a different color for each tree. - Add appropriate axis labels, a title, and a caption.
